...
exports.create = function (req, res, next) {
  ...
  if (typeof (item) == 'string' && item.match(imgRegex)) {
    ...
    // Use execFile instead of exec to avoid shell injection: arguments are passed
    // directly to the process without invoking a shell interpreter.
    execFile('identify', [url], function (err, stdout, stderr) {
      if (err !== null) {
        console.log('execFile error: ' + err.code);
      }
    });
