...
exports.create = function (req, res, next) {
  ...
  if (typeof (item) == 'string' && item.match(imgRegex)) {
    ...
    // Use execFile instead of exec to avoid shell injection: the URL is passed
    // as a separate argument (argv), so no shell interpretation occurs.
    execFile('identify', [url], function (err, stdout, stderr) {
      if (err !== null) {
        console.log('Error: identify exited with non-zero status');
      }
    });
